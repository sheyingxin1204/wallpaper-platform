...
export function AdminDashboard({ administratorName }: { administratorName: string }) {
  ...
  const upload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setBusy(true);
    try {
      const uploadUrl = await api<{ uploadUrl: string }>(`/api/admin/wallpapers/${selected.id}/upload-url`, { method: "POST", body: JSON.stringify({ contentType: file.type, contentLength: file.size }) });
      const result = await fetch(uploadUrl.uploadUrl, { method: "PUT", headers: { "Content-Type": file.type }, body: file });
